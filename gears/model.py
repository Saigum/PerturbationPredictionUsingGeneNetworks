...
class GEARS_EMBED(GEARS_Model):
    def __init__(self, args):
        super().__init__(args)
        self.gene_emb = nn.Linear(self.num_genes,args["hidden_size"])
    def forward(self, data):
        """
        Forward pass of the model
        """
        ...
        if self.no_perturb:
            ...
            ## get base gene embeddings, num_batch of the same graph.
            ## reshape 
            emb = self.gene_emb(x.view(-1,self.num_genes)) ## experiment where we try to consider the expression values in the embedding as well.
            print(x.view(-1,self.num_genes).shape)
            print(emb.shape)
            emb = self.bn_emb(emb)
            base_emb = self.emb_trans(emb)        
